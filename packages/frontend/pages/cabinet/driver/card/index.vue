    ...
    <n-card v-else>
      <n-tabs type="line" animated>
        <n-tab-pane name="main" tab="1. Основная информация">
          <n-form :model="form" label-placement="top" style="max-width: 600px; padding-top: 16px;">
            <n-form-item label="Водитель ID">
              <n-input :value="form.id" disabled placeholder="Генерируется системой" />
            </n-form-item>
            <n-form-item label="ФИО">
              <n-input v-model:value="form.full_name" placeholder="Мурадов Азат" />
            </n-form-item>
            <n-form-item label="Дата рождения">
              <n-input v-model:value="form.date_of_birth" type="date" placeholder="15.04.1988" />
            </n-form-item>
            <n-form-item label="Гражданство">
              <n-input v-model:value="form.citizenship" placeholder="Туркменистан" />
            </n-form-item>
            ...
            <n-form-item label="Контактный телефон">
              <n-input v-model:value="form.phone" placeholder="+993 65 12 34 56" />
            </n-form-item>
            <n-form-item label="Email">
              <n-input :value="form.email" disabled placeholder="Из учётной записи" />
            </n-form-item>
        ...
        <n-tab-pane name="passport" tab="2. Паспортные данные">
          <n-form :model="form" label-placement="top" style="max-width: 600px; padding-top: 16px;">
            ...
            <n-form-item label="Дата выдачи">
              <n-input v-model:value="form.passport_issue_date" type="date" placeholder="12.05.2018" />
            </n-form-item>
            <n-form-item label="Дата окончания">
              <n-input v-model:value="form.passport_expiry_date" type="date" placeholder="12.05.2028" />
            </n-form-item>
        ...
        <n-tab-pane name="license" tab="3. Водительское удостоверение">
          <n-form :model="form" label-placement="top" style="max-width: 600px; padding-top: 16px;">
            ...
            <n-form-item label="Дата выдачи">
              <n-input v-model:value="form.license_issue_date" type="date" placeholder="10.04.2019" />
            </n-form-item>
            <n-form-item label="Дата окончания">
              <n-input v-model:value="form.license_expiry" type="date" placeholder="10.04.2029" />
            </n-form-item>
            ...
            <n-form-item label="Дата последнего медосмотра">
              <n-input v-model:value="form.last_medical_examination_date" type="date" placeholder="01.09.2025" />
            </n-form-item>
        ...
        <n-tab-pane name="permits" tab="4. Разрешительные документы">
          <n-form :model="form" label-placement="top" style="max-width: 600px; padding-top: 16px;">
            ...
            <n-form-item label="Дата выдачи разрешения">
              <n-input v-model:value="form.permission_issue_date" type="date" placeholder="01.06.2024" />
            </n-form-item>
            <n-form-item label="Срок действия">
              <n-input v-model:value="form.permission_validity_date" type="date" placeholder="01.06.2025" />
            </n-form-item>
...
const passportFileList = computed(() =>
  form.passport_scan_url
    ? [{ id: '1', name: 'Скан паспорта', status: 'done' as const, url: `${API}${form.passport_scan_url}` }]
    : [],
)
...
async function handlePassportUpload({ file, onFinish, onError }: { file: { file: File }; onFinish: () => void; onError: (e: Error) => void }) {
  try {
    ...
    const res = await $fetch<{ url: string }>(`${API}/cabinet/driver/profile/upload-passport`, {
      method: 'POST',
      credentials: 'include',
      body: fd,
    })
...
const licenseFileList = computed(() =>
  form.license_scan_url
    ? [{ id: '1', name: 'Скан прав', status: 'done' as const, url: `${API}${form.license_scan_url}` }]
    : [],
)
...
async function handleLicenseUpload({ file, onFinish, onError }: { file: { file: File }; onFinish: () => void; onError: (e: Error) => void }) {
  try {
    const fd = new FormData()
    fd.append('file', file.file, file.file.name)
    const res = await $fetch<{ url: string }>(`${API}/cabinet/driver/profile/upload-license`, {
      method: 'POST',
      credentials: 'include',
      body: fd,
    })
...
const medicalFileList = computed(() =>
  form.medical_certificate_scan_url
    ? [{ id: '1', name: 'Скан мед. справки', status: 'done' as const, url: `${API}${form.medical_certificate_scan_url}` }]
    : [],
)
...
async function handleMedicalUpload({ file, onFinish, onError }: { file: { file: File }; onFinish: () => void; onError: (e: Error) => void }) {
  try {
    const fd = new FormData()
    fd.append('file', file.file, file.file.name)
    const res = await $fetch<{ url: string }>(`${API}/cabinet/driver/profile/upload-medical`, {
      method: 'POST',
      credentials: 'include',
      body: fd,
    })
...
async function loadProfile() {
  ...
  try {
    const data = await $fetch<any>(`${API}/cabinet/driver/profile`, { credentials: 'include' })
    form.id = data.id ?? ''
...
async function handleSave() {
  saving.value = true
  try {
    await $fetch(`${API}/cabinet/driver/profile`, {
      method: 'PATCH',
      credentials: 'include',
      body: {
