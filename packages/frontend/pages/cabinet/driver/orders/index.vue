...
const columns = computed<DataTableColumns<any>>(() => [
  ...
  { title: t('common.from'), key: 'fromCity', render: row => (row.fromRegion ? row.fromRegion + ', ' : '') + (row.fromCity || row.from_city), ellipsis: true },
  { title: t('common.to'), key: 'toCity', render: row => (row.toRegion ? row.toRegion + ', ' : '') + (row.toCity || row.to_city), ellipsis: true },
