...
import { eq, and, inArray, desc, ilike, sql, gte, lte } from 'drizzle-orm';
...
export const publicOrdersRoutes = new Elysia({ prefix: '/public/orders' })
  .get('/', async ({ query }) => {
    ...
    if (params.fromCity) {
      conditions.push(ilike(orders.fromCity, `%${params.fromCity}%`));
    }
    if (params.toCity) {
      conditions.push(ilike(orders.toCity, `%${params.toCity}%`));
    }
    if (params.fromCountry) {
      conditions.push(eq(orders.fromCountry, params.fromCountry));
    }
    if (params.toCountry) {
    ...
    if (params.weightMin) {
      conditions.push(sql`${orderCargo.weightKg} >= ${parseFloat(params.weightMin)}`);
    }
    if (params.weightMax) {
      conditions.push(sql`${orderCargo.weightKg} <= ${parseFloat(params.weightMax)}`);
    }
    if (params.volumeMin) {
      conditions.push(sql`${orderCargo.volumeM3} >= ${parseFloat(params.volumeMin)}`);
    }
    if (params.volumeMax) {
      conditions.push(sql`${orderCargo.volumeM3} <= ${parseFloat(params.volumeMax)}`);
    }
    if (params.packaging) {
      conditions.push(eq(orderCargo.packaging, params.packaging));
    }
    if (params.readyFrom) {
      conditions.push(sql`${orders.readyDate} >= ${params.readyFrom}`);
    }
