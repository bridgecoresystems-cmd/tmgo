...
**Изменил `schema.ts` → сразу запусти `bun run db:generate` → закоммить оба файла вместе.**

Если этого не сделать — после деплоя бэкенд упадёт с 500 на всех эндпоинтах, которые обращаются к новым колонкам/таблицам.

### Чеклист перед `git push`

- Менял `schema.ts`? → есть новый файл в `drizzle/`?
- `git status` показывает оба файла в коммите?

### Что будет если забыть

Сервер вернёт 500. DevOps добавит колонку/таблицу руками через `psql`. Но это технический долг — нормальная миграция всё равно нужна потом.

## Сиды справочников

Миграции создают **пустые** таблицы. Справочники (марки/модели ТС, города и т.п.) наполняются отдельными сид-командами — они **не входят** в `db:migrate`.

После любого пересоздания БД (drop + миграции с нуля) или клонирования проекта на новую машину/VPS — запусти **все** сиды:

```bash
cd packages/backend
bun run seed:vehicle-makes   # 46 марок + 162 модели (фуры, тягачи, прицепы)
# другие seed:* — см. package.json
```

Сиды **идемпотентны** (проверяют slug перед вставкой) — повторный запуск ничего не сломает.

### Симптом пропущенного сида

Каталог в форме создания ТС/прицепа пустой, `GET /cabinet/driver/vehicles/makes` возвращает `[]`. Проверить:
```bash
PGPASSWORD=... psql -h localhost -U tmgo_user -d tmgo -c "SELECT count(*) FROM vehicle_makes;"
```
Если 0 — сид не выполнялся.
